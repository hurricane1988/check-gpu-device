...
docker-build: ## Build docker image with the check-nvidia-cuda.
	$(CONTAINER_TOOL) build -t ${IMG}:${VERSION} -f Dockerfile .
...
docker-buildx: ## Build and push docker image for the check-gpu-check for cross-platform support.
	- $(CONTAINER_TOOL) buildx create --name check-gpu-check
	$(CONTAINER_TOOL) buildx use check-gpu-check
	- $(CONTAINER_TOOL) buildx build --push --platform=$(PLATFORMS) --tag ${IMG}:${VERSION} -f Dockerfile .
	- $(CONTAINER_TOOL) buildx rm check-gpu-check
